...
public sealed class UpdateService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(serverSetupPath) || !File.Exists(serverSetupPath))
        {
            return null;
        }
    ...
    /// <summary>
    /// Mikro uygulamasını başlatır.
    /// </summary>
    public void LaunchMikro(string exePath)
    {
        ArgumentNullException.ThrowIfNull(exePath);

        if (!File.Exists(exePath))
        {
            throw new FileNotFoundException("Mikro EXE dosyası bulunamadı.", exePath);
        }
    ...
    /// <summary>
    /// Geçici indirme dizinini temizler.
    /// </summary>
    public static void CleanupTempFiles()
    {
        if (Directory.Exists(TempDirectory))
        {
            ...
            {
                // Dosyalar kullanımda olabilir, yoksay
            }
        }
    }
}
